...
const BookChat = () => {
    ...
    const [selectedNotebookId, setSelectedNotebookId] = useState(() => {
        ...
        const sortedNbs = nbs.sort((a, b) => new Date(b.lastModified || 0) - new Date(a.lastModified || 0));
        return sortedNbs.length > 0 ? sortedNbs[0].id : '';
    });
    ...
    const handlePurge = () => {
        toast.warn(
            <ScholarlyConfirm 
                ...
                message="Are you certain, Scholar? This will permanently erase the entire chat history from the records."
                actionLabel="Purge"
                onConfirm={clearChat}
            />,
            {
        ...
        <div className="flex w-full h-full bg-[#F4EFE6] text-[#3C2A21] font-serif overflow-hidden">
            ...
            <main className="flex-1 flex flex-col items-center overflow-y-auto px-4 py-12 custom-scrollbar">
                <div className="w-full max-w-2xl space-y-12 mb-32">
                    <header className="border-b border-[#3C2A21]/10 pb-8 mb-12">
                        <p className="text-[0.6rem] font-bold uppercase tracking-[0.3em] text-stone-500 mb-1">Academic Intelligence</p>
            ...
            <aside className="w-[20vw] border-l border-[#3C2A21]/10 bg-[#F4EFE6]/50 p-6 flex flex-col gap-10 overflow-y-auto hidden xl:flex shrink-0">
                <div className="pb-4 border-b border-[#3C2A21]/5">
                    <h2 className="text-[0.8rem] font-black uppercase tracking-[0.3em] text-stone-400">Assitant Context Panel</h2>
                </div>
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-[1rem] font-black uppercase tracking-[0.2em]">Target Notebook</h3>
                    </div>
                    ...
                    <div className="p-3 bg-[#5D2E2E]/5 border border-[#5D2E2E]/10 rounded-sm">
                        <p className="text-[0.6rem] font-bold text-[#5D2E2E] uppercase mb-1 opacity-60 font-sub">Active Archive Target</p>
                        {selectedNotebookId ? (
                            <Link 
                                to={`/notebooks/${state.notebooks[selectedNotebookId]?.subjectId}/${selectedNotebookId}`}
                                className="text-s font-bold truncate hover:text-[#5D2E2E] hover:underline transition-all block"
                            >
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm opacity-60"></span>
                        <h3 className="text-[1rem] font-black uppercase tracking-[0.2em]">Reference Context</h3>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                        {taggedNoteIds.map(id => (
                            <div key={id} className="flex items-center justify-between p-2 bg-white border border-[#3C2A21]/5 rounded-sm group">
                                <Link 
                                    to={`/notes/${id}`}
                                    className="text-[0.7rem] truncate max-w-[80%] hover:text-[#5D2E2E] hover:underline transition-all"
                                >
                                    {getNoteTitle(id)}
                                </Link>
